...
export default function Home() {
  ...
  const [queryKey, setQueryKey] = useState(0); // This will trigger the re-render of the summary and timeline
  ...
  const handleSearch = async () => {
    ...
    const data = await res.json();
    const summaryText = data.summary ? data.summary.replace(/^\s*-\s*/gm, '') : 'No summary found.';
    ...
    setQueryKey(prevKey => prevKey + 1);  // Increment the key for re-render
  };
  ...
  return (
    <main className="p-8 max-w-3xl mx-auto">
          ...
          <ul className="pl-5">
            {summary
              ...
              .map((line, index) => (
                <li
                  key={index}
                  className="list-disc fade-in-summary"
                  style={{ animationDelay: `${index * 0.5}s` }} // Delay each bullet point
                >
                  {line.trim()}
                </li>
              ))}
      ...
      {sortedTimeline.length > 0 && (
        <div className="mt-6" key={queryKey}> {/* Key added here to force re-render */}
